...
use app\models\AxionClaimsurveyHistory;
use app\models\PreinspectionClientCaller;
use app\models\EmailHistory;
use app\models\AxionClaimsurvey;
use app\models\AxionClaimsurveySearch;
use app\models\AxionClaimsurveyFourwheeler;
use app\models\AxionClaimsurveyMetalicparts;
use app\models\AxionClaimsurveyLabourchrefitparts;
use app\models\AxionClaimsurveyAssessment;
use app\models\AxionClaimsurveyBill;
use app\models\AxionClaimsurveyPhotos;
...
use app\models\AxionPreinspectionApi;
...
/**
 * ClaimsurveyController implements the CRUD actions for Claimsurvey model.
 */
class AxionClaimsurveyController extends Controller
{
    ...
    public function actionIndex()
    {
         ...
         */ 
               ...
               // your default model and dataProvider generated by gii
                $searchModel = new AxionClaimsurveySearch();
                ...
                if(isset($params['AxionClaimsurveySearch']['insurerBranch']))
                {
                    
                    $branch = PreinspectionClientBranch::find()
                        ->where(['companyId' => $params['AxionClaimsurveySearch']['insurerName']])
                        ->all();  
    ...
    public function actionTransaction($id)
    {
        ...
        $query = AxionClaimsurveyHistory::find()->where(['preinspection_id' => $id]);
    ...
    {
        if(Image::getImagine()->open($inputFile)->thumbnail(new Box(500, 500))->save($inputFile , ['quality' => 70]))
        {
           if($displayText != '')
            {
    ...
    protected function saveImageObj($obj,$id,$lastImage,$img) {
        ...
        $image_base64 = base64_decode($image_parts[1]);
            if($image_base64)
            {
                ...
                if(file_put_contents($inputFile, $image_base64))
                {
                    ...
                    if($lastImage != '')
                    {
                        $filename = getcwd().'/'.$csqcLoc.$lastImage;
    ...
    public function actionImageUpload()
    {
       ...
       // $qcLoc = \Yii::$app->params['qcLoc'];
        ...
        $obj = AxionClaimsurveyPhotos::findOne(['preinspection_id' => $id,'type' => $type]);
        $locStatusData = $obj->locStatusValue;
        
        $timeStatusData = $obj->timeStatusValue;
        
         $premodel = AxionClaimsurvey::findOne($id);
        ...
        $lastImage = '';
                ...
                {
                    $image = $this->saveImageObj($obj,$id,$lastImage,$img);
                }
                $imgUrl = Yii::$app->urlManager->createAbsoluteUrl($csqcLoc.$image);
                

                return $imgUrl;
    }
    
   protected function saveImageObjbrowse($obj,$id,$lastImage) {
    ...
    public function actionVideoSession($id)
    {
        ...
        if ($premodel->load(Yii::$app->request->post()) && $premodel->save()){
          
          $premodel = AxionClaimsurvey::findOne($id);
          $mobile = $premodel->shopNumber;
          $link = Yii::$app->urlManager->createAbsoluteUrl('axion-claimsurvey/video-session?id='.$id);
          $msg = 'Join Video Session With Customer Reference URL '.$link;            
      ...
      /*  $image = 'qcphotos/22-chassis.jpg';
        ...
        $premodel = AxionClaimsurvey::findOne($id); 
        $metalicPart = AxionClaimsurveyMetalicparts::findOne(['preinspection_id' => $id]);
        $labchPart = AxionClaimsurveyLabourchrefitparts::findOne(['preinspection_id' => $id]);
        $bill = AxionClaimsurveyBill::findOne(['preinspection_id' => $id]);
          

        // $count = count(Yii::$app->request->post('AxionClaimsurveyAssessment', []));
        ...
        $premodel->scenario = 'fourwheelerqc';

        $completedSurveyDateTime = $premodel->completedSurveyDateTime;
        $customerAppointDateTime = $premodel->customerAppointDateTime;
          
        $phmodel = AxionClaimsurveyPhotos::findAll(['preinspection_id' => $id]);
        if(count($phmodel) == 0)
        {
            $this->updateQc($premodel, 'insert');
            $phmodel = AxionClaimsurveyPhotos::findAll(['preinspection_id' => $id]);
        }
        $model = AxionClaimsurveyFourwheeler::findOne(['preinspection_id' => $id]);
        ...
        $division = PreinspectionClientDivision::find()
                ->where(['id' => $premodel->insurerDivision])
                ->all();
        $branch = PreinspectionClientBranch::find()
                ->where(['id' => $premodel->insurerBranch])
                ->all();            
       ...
       $token = $id.$premodel->insuredMobile;
       
       /*
       $customerSession = VideoRequests::find()
        ->where(['token' => $token])
        ->count(); */

        $customerSession = VideoRequests::findOne(['token'=>$token]);
        ...
        if (Yii::$app->request->post('create_session')==='create_session') {
           ...
           return $this->redirect(array('fourwheelerqc', 'id' => $id));
        ...
        else if ($premodel->load(Yii::$app->request->post()) && $model->load(Yii::$app->request->post())  && $metalicPart->load(Yii::$app->request->post()) && $labchPart->load(Yii::$app->request->post()) && $bill->load(Yii::$app->request->post())) {   
            ...
            (int)$bill->total = (int)$bill->particularAmount1 + (int)$bill->particularAmount2 + (int)$bill->particularAmount3 + (int)$bill->particularAmount4 + 
                                (int)$bill->particularAmount5 + (int)$bill->particularAmount6 + (int)$bill->particularAmount7 + (int)$bill->particularAmount8;
            (float)$gst = (float)($bill->total / 100 * 9);
            (float)$bill->cgst = (float)$gst;
            (float)$bill->sgst = (float)$gst;

            (float)$bill->roundedOffTo = (float)$bill->total + (float)$bill->sgst + (float)$bill->cgst;
            ...
            if($this->saveFourwheelerpdf($id))
            {
                $this->savePhotosPdf($premodel->id);
            }
    ...
    public function actionFourwheelerpdf($id) {
        ...
        'options' => [
            'title' => $surveyor_name,
            'subject' => 'Claimsurvey Report'
        ],
        'methods' => [
            'SetHeader' => [$surveyor_name.'||Claimsurvey Report'],
...
protected function saveFourwheelerpdf($id) {
    ...
    {
       $model = AxionClaimsurvey::findOne($id);       
    ...
    {
        $phmodel = new AxionClaimsurveyPhotos();
    ...
    protected function updateQc($preModel,$updateType)
    {
        if($preModel->vehicleType == '4-WHEELER')
        {
            if($updateType == 'insert')
            {
                $countPosts = AxionClaimsurveyFourwheeler::find()
                        ->where(['preinspection_id' => $preModel->id])
                        ->count();

                if($countPosts == 0)
                {
                    $model = new AxionClaimsurveyFourwheeler();
                    $model->preinspection_id = $preModel->id;
                    $model->save();
                    
                    $this->insertQcImageRecord($preModel->id, 'Photo-01');
                    $this->insertQcImageRecord($preModel->id, 'Photo-02');
                    $this->insertQcImageRecord($preModel->id, 'Photo-03');
                    $this->insertQcImageRecord($preModel->id, 'Photo-04');
                    $this->insertQcImageRecord($preModel->id, 'Photo-05');
                    $this->insertQcImageRecord($preModel->id, 'Photo-06');
                    $this->insertQcImageRecord($preModel->id, 'Photo-07');
                    $this->insertQcImageRecord($preModel->id, 'Photo-08');
                    $this->insertQcImageRecord($preModel->id, 'Photo-09');
                    $this->insertQcImageRecord($preModel->id, 'Photo-10');
                    $this->insertQcImageRecord($preModel->id, 'Photo-11');
                    $this->insertQcImageRecord($preModel->id, 'Photo-12');
                    $this->insertQcImageRecord($preModel->id, 'Photo-13');
                    $this->insertQcImageRecord($preModel->id, 'Photo-14');
                    $this->insertQcImageRecord($preModel->id, 'Photo-15');
                    $this->insertQcImageRecord($preModel->id, 'Photo-16');
                    $this->insertQcImageRecord($preModel->id, 'Photo-17');
                    $this->insertQcImageRecord($preModel->id, 'Photo-18');
                    $this->insertQcImageRecord($preModel->id, 'Photo-19');
                    $this->insertQcImageRecord($preModel->id, 'Photo-20');
                    $this->insertQcImageRecord($preModel->id, 'Photo-21');
                    $this->insertQcImageRecord($preModel->id, 'Photo-22');
                    $this->insertQcImageRecord($preModel->id, 'Photo-23');
            ...
            if($updateType == 'delete')
            {
                 AxionClaimsurveyFourwheeler::deleteAll(['preinspection_id' => $preModel->id]);
                 AxionClaimsurveyPhotos::deleteAll(['preinspection_id' => $preModel->id]);
            }
        }
    ...
    {
        $acmodel = AxionClaimsurvey::findOne($id);

        $token = $id.$acmodel->insuredMobile;

        if(VideoRequests::findOne(['token'=>$token])) {
          $model = VideoRequests::findOne(['token'=>$token]);
        } else {
          $model = new VideoRequests();
          date_default_timezone_set('Asia/Calcutta');
        ...
        $model->tsComp = $acmodel->id;
        //$model->tsType = $attributes['tsType'];
        //$model->tsTxnRefNo = $attributes['tsTxnRefNo'];
        //$model->tsRefId = $attributes['tsRefId'];

        //$model->tsRefSrNo = $attributes['tsRefSrNo'];
        $model->tsCustName = $acmodel->insuredName;
        //$model->tsCustRefNo = $attributes['tsCustRefNo'];
        $model->tsMobileNo = $acmodel->insuredMobile;
        $model->tsEmailId = $acmodel->contactPersonMobileNo;
        $model->tsCrUid = Yii::$app->user->identity->id;
        $model->securitykey = $this->getGUID();
        $model->token = $token;
        ...
        /*
        if($model->save()) {
          // $link = Yii::$app->urlManager->createAbsoluteUrl('video-requests/startsession?sessionid='.$model->securitykey);
          // return array('sessionid'=>$model->securitykey, 'url'=>$link);
          // return $model->securitykey;
          return array('tsToken'=>$model->securitykey);
        } else {
          return array('error'=> 'Got an error while saving data');
        }
        */
       ...
       if($cmodel->save())
       {
            ...
            if($model->ro == '')
            {
               ...
               {
                    $link = Yii::$app->urlManager->createAbsoluteUrl('axion-claimsurvey/fourwheelerqc?id='.$model->id);
               }
               else
               {
                   $link = Yii::$app->urlManager->createAbsoluteUrl('axion-claimsurvey/commercialqc?id='.$model->id);
               }
    ...
    /**
     * Creates a new Claimsurvey model.
        ...
        if (Yii::$app->request->isAjax && $model->load(Yii::$app->request->post())) {
                ...
                if($model->surveyorName)
                {
                 ...
                 if($model->save())
                 {
                    ...
                    $metalicPart = new AxionClaimsurveyMetalicparts();
                    $metalicPart->userId = Yii::$app->user->identity->id; 
                    $metalicPart->preinspection_id = $model->id;
                    $metalicPart->lastUpdatedOn = date('Y-m-d h:i:s');
                    $metalicPart->save();
                    
                    $labchPart = new AxionClaimsurveyLabourchrefitparts();
                    $labchPart->userId = Yii::$app->user->identity->id; 
                    $labchPart->preinspection_id = $model->id;
                    $labchPart->lastUpdatedOn = date('Y-m-d h:i:s');
                    $labchPart->save();
                    
                    $bill = new AxionClaimsurveyBill();
                    $bill->preinspection_id = $model->id;
                    $bill->userId = Yii::$app->user->identity->id;
                    $bill->lastUpdatedOn = date('Y-m-d H:i:s');
                    $bill->save();
                    ...
                    $hismodel = new AxionClaimsurveyHistory();
    ...
    {
        if (($model = AxionClaimsurvey::find()->orderBy(['id' => SORT_DESC])->one()) !== null) {
    ...
    protected function getFollowupData($call,$idCheck,$ro)
    {
        ...
        $followup = AxionClaimsurvey::find()
    ...
    protected function hisMessage($id)
    {
        ...
        $his = AxionClaimsurveyHistory::find()
        ...
        foreach($his As $val)
        {
            ...
            {
                $msg .= '<tr><td style="padding:15px;border:1px solid #000;"> STATUS ON '.date( 'd/m/Y h:i A', strtotime( $val->created_on )).'</td><td style="padding:15px;border:1px solid #000;"> Spot Survey - DateTime :'.date( 'd/m/Y h:i A', strtotime( $val->completedSurveyDateTime )).'</td></tr>';
            }
            else if($val->status == 101)
            {
                $msg .= '<tr><td style="padding:15px;border:1px solid #000;"> STATUS ON '.date( 'd/m/Y h:i A', strtotime( $val->created_on )).'</td><td style="padding:15px;border:1px solid #000;"> Final-Survey </td></tr>';
            }
            else if($val->status == 102)
            {
                $msg .= '<tr><td style="padding:15px;border:1px solid #000;"> STATUS ON '.date( 'd/m/Y h:i A', strtotime( $val->created_on )).'</td><td style="padding:15px;border:1px solid #000;"> Re-Inspection </td></tr>';
            }
            else if($val->status == 103)
            {
                $msg .= '<tr><td style="padding:15px;border:1px solid #000;"> STATUS ON '.date( 'd/m/Y h:i A', strtotime( $val->created_on )).'</td><td style="padding:15px;border:1px solid #000;"> Inprogress </td></tr>';
            }
            else if($val->status == 104)
            {
                $msg .= '<tr><td style="padding:15px;border:1px solid #000;"> STATUS ON '.date( 'd/m/Y h:i A', strtotime( $val->created_on )).'</td><td style="padding:15px;border:1px solid #000;"> Initial-Survey </td></tr>';
            }
    ...
    /**
     * Updates an existing Claimsurvey model.
    ...
    public function actionDelete($id)
    {
        ...
        */
        ...
        if (!$id) {
          ...
          AxionClaimsurvey::findOne($id)->delete();
    ...
    public function actionDeleteMultiple()
    {
        ...
        {
          AxionClaimsurvey::findOne($value)->delete();
        }
        return;
    }
    ...
    public function actionRemovePhoto()
    {
        ...
        $csqcLoc = \Yii::$app->params['csqcLoc'];
            ...
            $model = AxionClaimsurveyPhotos::findOne($id);
            $filename = getcwd().'/'.$csqcLoc.$model->image;
            if(unlink($filename))
            {
                $model->image = '';
                $model->iLat = '';
    ...
    public function actionDownloadphotos($id)
    {
        ...
        $tmp_file = getcwd().'/api-uploads/'.$referenceNo.'-images.zip';
        ...
        if(count($phmodel) > 0)
        {
             ...
             $zip->close();
                ...
                $path = getcwd().'/api-uploads/'.$referenceNo.'-images.zip';
                if(file_exists($path)){
    ...
    protected function savePhotosPdf($id)
    {
        $model = AxionClaimsurvey::findOne($id);
        $referenceNo = $model->referenceNo;
        $phmodel = AxionClaimsurveyPhotos::findAll(['preinspection_id' => $id]);
        ...
        $tmp_file = getcwd().'/api-uploads/'.$referenceNo.'-images-pdf.zip';
    ...
    {
        if (($model = AxionClaimsurvey::findOne($id)) !== null) {
            return $model;
        } else {
    ...
    public function actionVehicleCheck($regno)
    {
            ...
            $countPosts = AxionClaimsurvey::find()
