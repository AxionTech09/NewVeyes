...
class AxionPreinspection extends \yii\db\ActiveRecord
{
    ...
    public function rules()
    {
        ...
        $rules = [
            ...
            [['referenceNo', 'manufacturingYear','stateId', 'cityId', 'townId', 'extraKM', 'surveyorName', 'paymentMode', 'cancellationReason', 'userId', 'followupReason','insurerName', 'insurerDivision', 'insurerBranch', 'callerName','taigStatusCode'], 'integer'],
            [['referenceNo'], 'unique', 'message' => 'referenceNo already exists'],
            [['intimationDate', 'surveyorAppointDateTime', 'rescheduleDateTime', 'rescheduleDateTime1', 'customerAppointDateTime', 'completedSurveyDateTime', 'followupRemainder', 'followupUpdatedDateTime', 'created_on','uploadSource','vehicleType','vehicleTypeRadio','vType_id', 'sbuCode', 'insuredMobileAlt', 'surveyDoneOn', 'updated_on','qcDoneOn','cancelledOn','fristRoid', 'oreder_id', 'pay_status', 'total_score', 'regionName', 'updatedBy', 'status'], 'safe'],
            ...
            [['insuredAddress', 'intimationRemarks', 'surveyLocation', 'surveyLocation2', 'rescheduleReason', 'rescheduleReason1', 'remarks', 'ro','vTypeName','uploadPath','ErrorDesc','customerOccupation','residence','customerAge','numberOfCarsOwned','vehicleParked','securityOfVehicle','relationship','maintenance','vehicleTimeOfInspection','updatedContact'], 'string', 'max' => 255],
            ...
            [['remarks', 'callerName','inspectionType','insuredName',], 'required', 'on'=>['create', 'update']],
    ...
    public function getDamageName($id) 
    {
        $damageName = MasterDamageType::find()->where(['id' => $id])->one();
        return $damageName->damage_name;
    }
}
