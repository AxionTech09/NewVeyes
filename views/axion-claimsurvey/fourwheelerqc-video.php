    ...
    <div id="inspection_photos" class="preinspection-box" style="margin-bottom: 30px">
        ...
        foreach($phmodel as $obj)
        {
            ...
            echo $form->field($obj, 'image['.$obj->type.']')->widget(FileInput::classname(), [
                        ...
                        'uploadUrl' => Url::to(['/axion-claimsurvey/image-upload']),
                        'uploadExtraData' => [
                            'id' => $obj->preinspection_id,
                            'type'=> $obj->type,
                        ],
                        'initialPreview' => [
            $obj->image ? Html::img('@web/'.$csqcLoc.$obj->image,["width"=>"170px","height"=>"200px"]) : null, // checks the models to display the preview
        ],
            ...
            if($obj->image != '')
            {
                ...
                echo Html::img('@web/'.$csqcLoc.$obj->image,["width"=>"200px","height"=>"200px"]);
                echo Html::a('Remove', '#', ['class' => 'btn btn-primary remove-image', 'id'=>$obj->id]);
                echo "\x20\x20\x20";
                echo Html::a('ViewImage','@web/'.$csqcLoc.$obj->image, ['target' => '_blank']);
                if($obj->locStatus != 0)
