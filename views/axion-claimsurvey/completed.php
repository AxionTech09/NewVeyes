    ...
    //'template' => '{view} {update} {delete}',
     ...
     'buttons' => [
            ...
            'fourwheelerqc' => function ($url, $model, $key) {
                ...
                {
                   return Html::a('<span class="glyphicon glyphicon-check msize"></span>',Yii::$app->request->baseUrl.'/axion-claimsurvey/commercialqc?id='.$key, [
                    'class' => 'activity-transaction-link',
                //  'title' => Yii::t('yii', 'QC'),
                    'data-id' => $key,
                    'data-pjax' => '0',
            ...
            'fourwheelerpdf' => function ($url, $model, $key) {
                if($model->status == 101 || $model->status == 102 || $model->status == 104) 
                {
                    if(strtolower(preg_replace('/[^\w]/', '', $model->vehicleType)) == '4wheeler')
                {
                    return " ".Html::a('<span class="glyphicon glyphicon-download msize"></span>',Yii::$app->request->baseUrl.'/axion-claimsurvey/fourwheelerpdf?id='.$model->id, [
                        'class' => 'activity-transaction-link',
                ...
                {
                   return " ".Html::a('<span class="glyphicon glyphicon-download msize"></span>',Yii::$app->request->baseUrl.'/axion-claimsurvey/commercialpdf?id='.$model->id, [
                    'class' => 'activity-transaction-link',
                    'title' => Yii::t('yii', 'Download Report'),
                    'data-id' => $key,
                    'data-pjax' => '0',
            ...
            'downloadphotos' => function ($url, $model, $key) {
                if($model->status == 101 || $model->status == 102 || $model->status == 104) 
                {
                    return Html::a('<span class="glyphicon glyphicon-download-alt msize"></span>',Yii::$app->request->baseUrl.'/axion-claimsurvey/downloadphotos?id='.$key, [
                        'class' => 'activity-transaction-link',
...
[
    ...
    echo GridView::widget([
        ...
        'export'=>[
            ...
            'label'=>'Export',
            'header'=>'<li role="presentation" class="dropdown-header">Axion Claimsurvey Completed Records</li>',
        ],
