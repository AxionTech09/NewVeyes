...
$gridColumns = [
	...
	//'template' => '{view} {update} {delete}',
        'template' => Helper::filterActionColumn('{vehicleqc}{vehicleqcpdf}{downloadphotos}'),
	 ...
	 'buttons' => [
            ...
            'vehicleqc' => function ($url, $model, $key) {
             ...
             if(strtolower(preg_replace('/[^\w]/', '', $model->vehicleType)) == '4wheeler')
                {
                return Html::a('<span class="glyphicon glyphicon-check msize"></span>',Yii::$app->request->baseUrl.'/axion-preinspection/fourwheelerqc?id='.$key, [
                    'class' => 'activity-transaction-link',
                    'title' => Yii::t('yii', 'QC'),
                    'data-id' => $key,
                    'data-pjax' => '0',
            ...
            'vehicleqcpdf' => function ($url, $model, $key) {
                if($model->status == 101 || $model->status == 102 || $model->status == 104) 
                {
                    if(strtolower(preg_replace('/[^\w]/', '', $model->vehicleType)) == 'allvehicle')
                {
                    return " ".Html::a('<span class="glyphicon glyphicon-download msize"></span>',Yii::$app->request->baseUrl.'/axion-preinspection/vehicleqcpdf?id='.$model->id, [
                        'class' => 'activity-transaction-link',
                ...
                {
                   return " ".Html::a('<span class="glyphicon glyphicon-download msize"></span>',Yii::$app->request->baseUrl.'/axion-preinspection/commercialpdf?id='.$model->id, [
                    'class' => 'activity-transaction-link',
                    'title' => Yii::t('yii', 'Download Report'),
                    'data-id' => $key,
                    'data-pjax' => '0',
                    'target'=>'_blank',
                     ...
                     if(strtolower(preg_replace('/[^\w]/', '', $model->vehicleType)) == '4wheeler')
                {
                    return " ".Html::a('<span class="glyphicon glyphicon-download msize"></span>',Yii::$app->request->baseUrl.'/axion-preinspection/fourwheelerpdf?id='.$model->id, [
                        'class' => 'activity-transaction-link',
                        'title' => Yii::t('yii', 'Download Report'),
                        'data-id' => $key,
                        'data-pjax' => '0',
            ...
            'downloadphotos' => function ($url, $model, $key) {
                if($model->status == 101 || $model->status == 102 || $model->status == 104) 
                {
                    return Html::a('<span class="glyphicon glyphicon-download-alt msize"></span>',Yii::$app->request->baseUrl.'/axion-preinspection/downloadphotos?id='.$key, [
                        'class' => 'activity-transaction-link',
