...
$gridColumns = [
    ...
    'buttons' => [
            ...
            'transaction' => function ($url, $model, $key) {
                ...
                if($role!="Surveyor")
                {
                    return Html::a(FA::icon('history', ['class' => 'mr-5 mt-5 text-primary']), Yii::$app->request->baseUrl.'/axion-preinspection-autoqc/transaction?id='.$key, [
            ...
            'vehicleqc' => function ($url, $model, $key) {
                ...
                $role = array_keys(Yii::$app->authManager->getRolesByUser(Yii::$app->user->getId()))[0] ;
                if($role=="Admin" || $role=="Superadmin" || $role == 'Veyes UAT')
                {
                ...
                {
                    return '<div class="row">'.Html::a(FA::icon('user-check text-darkYellow', ['class' => 'mr-5 mt-5']), Yii::$app->request->baseUrl.'/axion-preinspection-autoqc/vehicleqc?id='.$key.'&page=completed', [
                ...
                {
                    return '<div class="row">'.Html::a(FA::icon('user-check text-darkYellow', ['class' => 'mr-5 mt-5']), Yii::$app->request->baseUrl.'/axion-preinspection-autoqc/commercialqc?id='.$key, [
                        'class' => 'activity-vehicleqc-link',
                        'title' => Yii::t('yii', 'QC'),
                        'data-id' => $key,
                        'data-pjax' => '0',
            ...
            'vehicleqcpdf' => function ($url, $model, $key) {
                ...
                if($model->status == 101 || $model->status == 102 || $model->status == 104) 
                {
                    if(strtolower(preg_replace('/[^\w]/', '', $model->vehicleType)) == 'allvehicle')
                    {
                        return " ".Html::a(FAS::icon('file-download', ['class' => 'mr-5 mt-5 text-limered']), Yii::$app->request->baseUrl.'/axion-preinspection-autoqc/vehicleqcpdf?id='.$model->id, [
                    ...
                    {
                    return " ".Html::a(FAS::icon('file-download', ['class' => 'mr-5 mt-5 text-limered']), Yii::$app->request->baseUrl.'/axion-preinspection-autoqc/commercialpdf?id='.$model->id, [
                        'class' => 'activity-vehicleqcpdf-link',
                        'title' => Yii::t('yii', 'Download Report'),
                        'data-id' => $key,
                        'data-pjax' => '0',
            ...
            'downloadphotos' => function ($url, $model, $key) {
                if($model->status == 101 || $model->status == 102 || $model->status == 104) 
                {
                    return Html::a(FAS::icon('images', ['class' => 'mr-5 mt-5 text-limegreen']), Yii::$app->request->baseUrl.'/axion-preinspection-autoqc/downloadphotos?id='.$key, [
                        'class' => 'activity-transaction-link',
...
<?php $this->registerJs(
    ...
    $.get(
        '".Yii::$app->request->baseUrl."/axion-preinspection-autoqc/create',
        {
...
<?php $this->registerJs(
    ...
    $.get(
        '".Yii::$app->request->baseUrl."/axion-preinspection-autoqc/update',
        {
            //id: $(this).closest('tr').data('key')
            id: $(this).data('id')
...
<?php $this->registerJs(
    ...
    $.get(
        '".Yii::$app->request->baseUrl."/axion-preinspection-autoqc/changerolist',
        {
            //id: $(this).closest('tr').data('key')
            id: $(this).data('id')
